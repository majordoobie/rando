...
doubled_odds = [n * 2 for n in numbers if n % 2 ==1 ]   #voodoo shit
  ...
  3) if criteria is True add it to the last function in this case multiply that shit by 2
...
wordlist = [x.upper() for x in list(open('file', 'r').readlines if 'u' in x]
  ...
  3) if true run it through the last function which is upper() method. 
